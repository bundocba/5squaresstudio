...
$doc->addStyleSheet(JUri::base().'templates/' . $this->template . '/css/lightbox.css');


$doc->addScript($this->baseurl . '/templates/' . $this->template . '/js/bootstrap.js', 'text/javascript');
$doc->addScript($this->baseurl . '/templates/' . $this->template . '/js/lightbox.js', 'text/javascript');
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="<?php echo $this->language; ?>" lang="<?php echo $this->language; ?>" dir="<?php echo $this->direction; ?>">
	...
	<body>
		...
		<?php endif ?>
		<div class="content_main container content_rs_form">
			<div class="row">
		...
		<script>
			jQuery(document).ready(function($) {
				...
				$(".rs-mail-right div.rsform-block-email div input").attr("placeholder", "your email is..");
				$(".rs-mail-right div div textarea").attr("placeholder", "and you want to");
				$(".rsform-block-companyname div input").attr("placeholder", "Company Name*");
				$(".rsform-block-companyaddress div input").attr("placeholder", "Company Address");
				$(".rsform-block-name div input").attr("placeholder", "Name*");
				$(".rsform-block-email div input").attr("placeholder", "Email*");
				$(".rsform-block-contact div input").attr("placeholder", "Contact*");
				$(".rsform-block-message div textarea").attr("placeholder", "Message*");
				$('.rs-mail-right .rsform-block-reset div input#reset').click(function(event) {
					 ...
					 $('.rs-mail-right div.rsform-block-email div textarea ').val("");
				});
			...
			(function($){
					$('#userForm').on('submit', function(){
						var isSubmit = function(){
							var Check = true;
							var para= $('.rsform-block-subject div select').val();
							if (para=="Subject*") {
								Check=false;
								alert('Please choose 1 Subject.');
							}
							return Check;
						};
						return isSubmit();
